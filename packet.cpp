...
Packet packetParser(String str){
  ...
  String node_id = str.substring(second_comma + 1, third_comma);
  String payload = str.substring(third_comma + 1, fourth_comma);
  long time_stamp = atol(str.substring(fourth_comma, str.length()).c_str());

  Packet p = {type, packet_id, node_id, payload, time_stamp};
